...
public final class Base64 {
     ...
     * >http://www.faqs.org/rfcs/rfc3548.html</a>. It is important to note that data encoded this way
     * is <em>not</em> officially valid Base64, or at the very least should not be called Base64
    ...
    public static byte[] decode(byte[] source, int off, int len, int options)
        ...
        if (len == 0) {
            ...
            throw new IllegalArgumentException(
                    "Base64-encoded string must have at least four characters, but length specified was " //$NON-NLS-1$
                            + len);
        } // end if
        ...
        for (i = off; i < off + len; i++) { // Loop through source
            ...
            if (sbiDecode >= WHITE_SPACE_ENC) {
                ...
                throw new java.io.IOException(String.format(
                        "Bad Base64 input character '%c' in array position %d", //$NON-NLS-1$
                        source[i], i));
            } // end else:
    ...
    //
    // // Open a stream
    // bis = new Base64.InputStream( new java.io.BufferedInputStream( new
    ...
    // // Set up some useful variables
    // java.io.File file = new java.io.File( filename );
    // byte[] buffer = new byte[Math.max( (int) ( file.length( ) * 1.4 ),
    // 40 )]; // Need max() for math on small files (v2.2.1)
    // int length = 0;
    // int numBytes = 0;
    //
    // // Open a stream
